...
function site_url(){
    ...
    if (!$site_url) {
        $site_url = rtrim(config('site.url'),'/').'/'; // Force forward slash
    }
    return $site_url;
}
...
// original config
// do not change key values here unless you intend to modify the actual config file
function config($key, $value = null) {
  ...
  if ($key === 'source' && file_exists($value)) {
    ...
    pref($key, $value);
  } else if ($key == 'source-write' && file_exists($value)) {
    $res = array();
    ...
    safe_file_rewrite($value, implode("\r\n", $res));
  } else if ($value == null) {
...
// similar to function config(key, value)
// Changes using config() will affect pref() but changes to pref() will not affect config()
// pref() values are also overwritten by any individual user preferences while config() is not
function pref($key, $value = null) {
    static $_pref = array();

    if ($key === 'source' && file_exists($value)) {
        $_pref = parse_ini_file($value, true);
        if (login()) {
            $user = $_SESSION['user'];
            if (file_exists(user_data_file($user, 'preferences'))) {
                $pref_data = parse_ini_file(user_data_file($user, 'preferences'), true);
                
                foreach ($pref_data as $pref_key => $pref_val) {
                    $_pref[$pref_key] = $pref_val;
                }
            }
        }
    } else if ($value == null) {
        return (isset($_pref[$key]) ? $_pref[$key] : null);
    } else {
        $_pref[$key] = $value;
    }
}
...
function partial($view, $locals = null) {
  ...
  if (($view_root = pref('views.root')) == null)
...
function route($method, $pattern, $callback = null) {
  ...
  if (!in_array($method, array('GET', 'POST', 'DELETE')))
    error(500, 'Only GET, DELETE and POST are supported');
